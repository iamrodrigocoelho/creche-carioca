...
const MUTABLE_TABLES = ['StatusEvent', 'AuditEvent', 'Application', 'Child', 'Guardian'] as const;
...
export async function resetEverything(prisma: PrismaClient): Promise<void> {
  await prisma.$executeRawUnsafe(
    `TRUNCATE TABLE "StatusEvent", "AuditEvent", "Application", "Child", "Guardian", "RuleVersion", "Process" RESTART IDENTITY CASCADE`,
  );
}
